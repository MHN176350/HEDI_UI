...
export default function LoginPage() {
  ...
  const { execute: loginUser, loading: isLoggingIn } = useApi(authService.login);
  const { execute: oauthLogin, loading: isGoogleLoading } = useApi(authService.oauthCallback);
    ...
    onSuccess: async (codeResponse) => {
      try {
        ...
        if (response && response.status === "SUCCESS") {
          ...
          setError(response?.message || "Google OAuth login failed");
        }
      } catch (err) {
        setError("OAuth connection to server failed.");
      }
    },
    onError: () => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#bee6ce] to-[#bcffdb] flex items-center justify-center p-6">
      <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <button type="submit" className="w-full py-3 bg-[#4f9d69] text-white rounded-lg hover:bg-[#68d89b] transition-colors font-medium shadow-md">
            Login
          </button>
        </form>
        ...
        <div className="mt-6">
         ...
         <button 
            ...
            className="mt-6 w-full py-3 border-2 border-[#4f9d69] text-[#4f9d69] bg-white rounded-lg hover:bg-[#bcffdb] transition-colors font-medium flex items-center justify-center gap-2 disabled:opacity-50"
          >
            {isGoogleLoading ? (
