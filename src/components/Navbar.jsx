...
export default function Navbar({ toggleSidebar }) {
  ...
  const location = useLocation(); // Listen for route changes to refresh notifications
                    ...
                    <div className="max-h-[320px] overflow-y-auto custom-scrollbar">
                      {notifications.length === 0 ? (
                        ...
                        notifications.map((notif) => (
                          <div key={notif.id} className={`p-4 border-b border-gray-50 transition-colors cursor-pointer flex gap-3 ${notif.read ? 'hover:bg-gray-50 opacity-70' : notif.type === 'WARNING' ? 'hover:bg-red-50/50 bg-red-50/20' : 'hover:bg-blue-50/50 bg-blue-50/10'}`}>
                            <div className={`w-2 h-2 mt-1.5 rounded-full shrink-0 shadow-sm ${notif.read ? 'bg-gray-300' : notif.type === 'WARNING' ? 'bg-red-500 shadow-red-200' : 'bg-blue-500 shadow-blue-200'}`}></div>
                            <div>
                              <p className="text-sm font-bold text-gray-800">
                                {notif.type === 'WARNING' ? 'Alert' : 'Update'}
                              </p>
